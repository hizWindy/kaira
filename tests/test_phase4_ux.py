...
def test_welcome_dashboard_outside_project(tmp_path, monkeypatch):
    """Welcome dashboard should print even when no .devflow.json exists."""
    monkeypatch.chdir(tmp_path)
    from devflow.commands.dashboard import welcome_dashboard
    with patch("devflow.commands.dashboard.console") as mock_console:
        welcome_dashboard()
...
def test_welcome_dashboard_inside_project(tmp_path, monkeypatch):
    """Welcome dashboard should show project details when .devflow.json exists."""
    monkeypatch.chdir(tmp_path)
    config = {
    ...
    (tmp_path / ".devflow.json").write_text(json.dumps(config))
    from devflow.commands.dashboard import welcome_dashboard
    with patch("devflow.commands.dashboard.console") as mock_console:
        welcome_dashboard()
        assert mock_console.print.called
