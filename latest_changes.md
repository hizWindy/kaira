...
## 1. Real Database Connection Diagnostics (`devflow db connect` / `status`)
...
## 2. New Table & Collection Inspector Command (`devflow db info`)

We introduced a new command, `devflow db info`, to display database schema details directly in your console:
...
* Previously, this caused CLI commands like `devflow db connect` or `devflow db status` to crash with a `DATABASE_URL not set` error.
...
## 4. Zero-Configuration Migrations (`devflow migrate`)

### The Problem
Previously, developers had to run `devflow migrate init` manually and edit `alembic/env.py` to import and register their database model classes. Missing this step resulted in empty migrations.

### The Solution
We made the database migration process **fully automated**:
* Running `devflow migrate make`, `devflow migrate run`, or `devflow migrate rollback` now dynamically checks for the presence of `alembic.ini` and `alembic/env.py` in your current directory.
...
* Running migrations is now a zero-setup task—just write your Python models and run `devflow migrate run`!

---

## 5. File Reload Protection & Windows Encoding Guard

* **Reload loop fix:** Running `devflow run` now configures `uvicorn` to exclude SQLite database files (`*.db`, `*.db-journal`, `*.db-wal`) and logs from file watcher monitoring. This prevents local database writes from triggering infinite application reloads.
