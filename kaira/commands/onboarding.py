...
def config_exists() -> bool:
    """Return True if the global Kaira user config file exists."""
    return _CONFIG_FILE.exists()


def load_config() -> dict[str, Any]:
    """Load and return the global Kaira user config dict.
...
def reset_config() -> None:
    """Delete the global Kaira user config so onboarding runs again."""
    try:
...
def run_onboarding() -> None:
    ...
    bolt = sym("BOLT")
    console.print(f"\n[{Theme.PRIMARY}]{bolt} Welcome to Kaira![/{Theme.PRIMARY}]\n")
    console.print(
        f"[{Theme.MUTED}]This one-time setup takes 30 seconds and can always be reset with:[/{Theme.MUTED}]"
        f"\n  [{Theme.PRIMARY}]kaira config reset-onboarding[/{Theme.PRIMARY}]\n"
    )
    ...
    try:
        experience = prompts.select(
            ...
            choices=[
                ("new", "New to FastAPI  → suggests kaira guide + tutorial tips"),
        ...
        telemetry = prompts.confirm(
            "Enable anonymous usage stats to improve Kaira?\n"
    ...
    if experience == "new":
        console.print(
            f"  [{Theme.MUTED}]Tip: run[/{Theme.MUTED}] [{Theme.PRIMARY}]kaira guide[/{Theme.PRIMARY}]"
