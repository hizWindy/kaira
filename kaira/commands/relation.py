...
def add_relation(
    ...
    if not file_exists(model_file):
        console.print(
            ...
            f"[dim]Run 'kaira generate model {model_a}' first.[/dim]"
        )
        raise typer.Exit(1)
    ...
    console.print(
        Panel(
            ...
            title="[bold]Kaira[/bold] — Adding Relationship",
            border_style="cyan",
        )
    )
    ...
    if relation_type == "many-to-many":
        ...
        console.print(
            ...
            f"[dim]   Run: kaira generate model {model_a} --fields ... to regenerate with the relation built-in.[/dim]"
        )
