"""Kaira cloud command group — cloud database providers and offline fallback.
...
Commands
--------
kaira cloud connect               Interactive wizard
kaira cloud status                Provider + latency + fallback state
kaira cloud test                  Round-trip health check
kaira cloud disconnect            Revert to local DB (typed confirm)
kaira cloud fallback status       Fallback mode + queued write count
kaira cloud fallback sync         Manual replay attempt
kaira cloud fallback enable       Wire up fallback into user project
kaira cloud fallback disable      Remove fallback wiring (typed confirm)
"""
...
def _add_to_gitignore(pattern: str) -> None:
    ...
    Path(".gitignore").write_text(f"# Kaira cloud\n{pattern}\n", encoding="utf-8")
...
_PROVIDER_HINTS = {
    ...
    "firebase": (
        ...
        "• Never commit this file to git — Kaira has added it to .gitignore."
    ),
}
    ...
    if provider is None:
        ...
        if provider not in _PROVIDERS:
            ...
            smart_error(
                ...
                fix_cmd="kaira cloud connect --provider supabase",
                guide_topic="cloud",
            )
            return
...
def cloud_status() -> None:
    ...
    if not is_cloud or provider not in _PROVIDERS:
        ...
        panel(
            ...
            f"Run [{Theme.PRIMARY}]kaira cloud connect[/{Theme.PRIMARY}] to connect to a cloud provider.",
            title="Cloud Status",
    ...
    if queue_count:
        console.print(
            f"  [{Theme.MUTED}]{arrow} kaira cloud fallback sync   (replay queued writes)[/{Theme.MUTED}]"
        )
...
def cloud_test() -> None:
    ...
    if not is_cloud or provider not in _PROVIDERS:
        warn = sym("WARN")
        console.print(
            f"[{Theme.WARNING}]{warn} No cloud database configured. Run kaira cloud connect first.[/{Theme.WARNING}]"
        )
        raise typer.Exit(1)
    ...
    if not success:
        error_footer(
            "Health check failed", hint=f"kaira cloud connect --provider {provider}"
        )
        raise typer.Exit(1)
...
def cloud_disconnect(
    ...
    --------
    kaira cloud disconnect
    kaira cloud disconnect --force   # skip confirmation (not in production)
    """
    cfg = _get_cloud_config()
    provider = cfg.get("cloud_provider") or cfg.get("database", "")
...
def fallback_status() -> None:
    ...
    if queue_count:
        console.print(
            f"  [{Theme.MUTED}]{arrow} kaira cloud fallback sync   (manual replay attempt)[/{Theme.MUTED}]"
        )
...
def fallback_sync() -> None:
    ...
    with spinner_context(f"Replaying {queue_count} queued writes..."):
        # This is the Kaira CLI tool — actual replay happens inside the
