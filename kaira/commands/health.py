...
def health_command() -> None:
    ...
    console.print("[cyan]⚡ Running Kaira Health Check...[/cyan]\n")
    ...
    if not auth_ok:
        suggestions.append(
            "Run [bold cyan]kaira auth generate --type jwt[/bold cyan] to generate authentication logic"
        )
    if not rate_limit_ok:
        suggestions.append(
            "Run [bold cyan]kaira auth generate[/bold cyan] to add rate limiting support"
        )
    if not docker_ok:
        suggestions.append(
            "Run [bold cyan]kaira docker init --with-compose[/bold cyan] to dockerise project"
        )
    if not ci_ok:
        suggestions.append(
            "Run [bold cyan]kaira ci generate --platform github[/bold cyan] to configure pipeline"
        )
    if not tests_ok:
        suggestions.append(
            "Run [bold cyan]kaira test generate --all[/bold cyan] to generate testing suite"
        )
