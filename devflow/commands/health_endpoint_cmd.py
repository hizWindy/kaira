"""DevFlow health-endpoint command group — /health route scaffolding.
...
def health_endpoint_generate() -> None:
    ...
    if cache_enabled:
        ...
        console.print("  [dim]ℹ️  Cache check not included (run devflow cache init to enable)[/dim]")
    ...
    if not cache_enabled:
        next_steps.append("[cyan]devflow cache init[/cyan] — enable cache check in /health")
    next_steps.append("[cyan]devflow api test GET /health[/cyan] — test the endpoint")
