"""Shared UX utilities for DevFlow CLI — used by all Phase 4 command modules."""
    ...
    if not config_path.exists():
        console.print(
            Panel(
                ...
                "  [bold cyan]devflow init <name>[/bold cyan]",
                title="[red]Not in a DevFlow project[/red]",
                border_style="red",
            )
        )
        raise typer.Exit(1)
...
def append_history(command: str, args: dict[str, str]) -> None:
    ...
    Args:
        command: The devflow command string (e.g. 'generate model').
