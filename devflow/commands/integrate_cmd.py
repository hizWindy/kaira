"""DevFlow integrate command group — third-party service integrations.
    ...
    if "/" not in category:
        console.print(
            Panel(
                ...
                "Example: devflow integrate --provider email/sendgrid[/red]",
                border_style="red",
            )
        )
        raise typer.Exit(1)
...
def integrate_list() -> None:
    ...
    table = Table(title="⚡ DevFlow — Available Integrations", border_style="cyan")
    ...
    console.print(table)
    console.print("[dim]Usage: devflow integrate --provider <category>/<provider>[/dim]")
