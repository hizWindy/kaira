"""DevFlow sync command group — cascade model field changes across all layers.

``devflow sync model`` closes the *field-refresh gap*: after a field is added to
    ...
    if all_models:
        ...
        if not targets:
            smart_error(
                ...
                fix_cmd='devflow generate model User --fields "name:str"',
        ...
        if not model_name:
            smart_error(
                ...
                fix_cmd="devflow sync model User",
                guide_topic="generate",
            )
...
def _sync_one(
    ...
    if entry is not None:
        ...
        smart_error(
            ...
            fix_cmd=f'devflow generate model {model_name} --fields "name:str"',
            guide_topic="generate",
        )
...
def _print_plan(
    ...
    if not is_document:
        body += f'\n\n  [{Theme.MUTED}]Tip: run [/{Theme.MUTED}][{Theme.PRIMARY}]devflow migrate make "sync {model_name.lower()}"[/{Theme.PRIMARY}] [{Theme.MUTED}]after applying.[/{Theme.MUTED}]'
