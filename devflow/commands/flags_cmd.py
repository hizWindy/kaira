"""DevFlow flags command group — feature flag management.
...
def _require_flags_file(output_root: Path) -> Path:
    ...
    if not fp.exists():
        console.print(
            Panel(
                "[yellow]core/flags.py not found. Run: [cyan]devflow flags init[/cyan] first.[/yellow]",
                border_style="yellow",
            )
        )
        raise typer.Exit(1)
...
def flags_init() -> None:
    ...
    if fp.exists():
        console.print("[yellow]core/flags.py already exists. Use devflow flags add to add flags.[/yellow]")
        return
    ...
    console.print(
        Panel(
            ...
            "Add flags with: [cyan]devflow flags add <flag_name> --default false[/cyan]",
            border_style="green",
        )
    )
...
def flags_enable(
    ...
    if flag_name not in flags:
        console.print(f"[red]❌ Flag '{flag_name}' not found. Add it with: devflow flags add {flag_name}[/red]")
        raise typer.Exit(1)
...
def flags_list() -> None:
    ...
    if not fp.exists():
        console.print("[yellow]No flags file found. Run: devflow flags init[/yellow]")
        return

    flags = _read_flags(fp)
    if not flags:
        console.print("[dim]No flags defined yet. Use devflow flags add <name>.[/dim]")
        return
