...
def seed_run(
    ...
    if not seeds_dir.exists():
        console.print("[yellow]No seeds directory found. Run devflow seed generate <Model> first.[/yellow]")
        return
...
def seed_clear() -> None:
    ...
    try:
        env = os.environ.copy()
        env["PYTHONPATH"] = str(output_root) + os.pathsep + env.get("PYTHONPATH", "")
        subprocess.run([sys.executable, "-c", clear_script], check=True, env=env)
