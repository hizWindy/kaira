"""DevFlow task command group — Celery background task scaffolding.
...
def task_init() -> None:
    ...
    console.print(
        Panel(
            ...
            "Next: [cyan]devflow task generate <TaskName>[/cyan] to create a task.",
            border_style="green",
        )
    )
...
def task_generate(
    ...
    console.print(
        Panel(
            ...
            f"Run with: [cyan]devflow task run {task_name}[/cyan]",
            border_style="green",
        )
    )
...
def task_list() -> None:
    ...
    if not tasks_dir.exists():
        console.print("[yellow]No tasks/ directory found. Run devflow task init first.[/yellow]")
        return

    task_files = [f for f in tasks_dir.glob("*.py") if f.name not in {"__init__.py", "celery_app.py"}]
    if not task_files:
        console.print("[dim]No tasks generated yet. Use devflow task generate <TaskName>.[/dim]")
        return
