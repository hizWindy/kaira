"""DevFlow status command — live project snapshot."""
...
def status_main(ctx: typer.Context) -> None:
    """Show a live snapshot of the DevFlow project status."""
...
def status_command() -> None:
    ...
    if not config_path.exists():
        console.print(
            Panel(
                "[yellow]No .devflow.json found. Run [bold]devflow init <name>[/bold] first.[/yellow]",
                border_style="yellow",
            )
        )
        return
    ...
    table = Table(title=f"⚡ DevFlow Status — {project_name}", border_style="cyan")
