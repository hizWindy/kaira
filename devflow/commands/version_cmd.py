...
def version_main_callback(ctx: typer.Context) -> None:
    """Show the DevFlow version when called without subcommands."""
    if ctx.invoked_subcommand is None:
        from devflow import __version__
        console.print(
            Panel(
                f"[bold cyan]DevFlow[/bold cyan] v[bold]{__version__}[/bold]",
                border_style="cyan",
            )
        )
...
def version_list() -> None:
    ...
    table = Table(title="DevFlow — API Versions", border_style="cyan")
