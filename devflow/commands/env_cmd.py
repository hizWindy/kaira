...
def env_validate() -> None:
    ...
    if not settings_path.exists():
        console.print(
            "[red]Error: config/settings.py not found. Run devflow env init first.[/red]"
        )
        raise typer.Exit(1)
...
def env_audit() -> None:
    ...
    if not rows:
        console.print(
            "[yellow]No .env.* files found. Run devflow env init first.[/yellow]"
        )
        return
    ...
    data_table(
        ...
        table_rows,
        title="⚡ DevFlow — Env Audit",
    )
    if unused:
        console.print(
            f"  [{Theme.WARNING}]{unused} unused key(s). Run [/{Theme.WARNING}]"
            f"[{Theme.PRIMARY}]devflow env prune[/{Theme.PRIMARY}]"
