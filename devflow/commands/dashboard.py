"""Welcome dashboard for DevFlow CLI — shown when devflow is run with no arguments."""
...
def _render_inside_project(config_path: Path) -> None:
    ...
    try:
        ...
        console.print(
            Panel(
                ...
                "Run [bold]devflow config show[/bold] to inspect it.[/yellow]",
                title="[yellow]DevFlow — Config Warning[/yellow]",
                border_style="yellow",
            )
        )
        return
    ...
    if db_type != "mongodb" and not (output_root / "alembic").exists():
        actions.append("Run [bold cyan]devflow migrate init[/bold cyan] to set up migrations")
    if not (auth_dir / "dependencies.py").exists():
        actions.append("Run [bold cyan]devflow auth generate --type jwt[/bold cyan] to add auth")
    if not (output_root / "Dockerfile").exists():
        actions.append("Run [bold cyan]devflow docker init --with-compose[/bold cyan] to add Docker")
    if router_count > test_count:
        actions.append("Run [bold cyan]devflow test generate --all[/bold cyan] to generate tests")

    # ── Header
    console.print(
        Panel(
            f"[bold cyan]⚡ DevFlow — {project_name}[/bold cyan]  "
    ...
    # ── Quick commands
    quick = (
        "[dim]Generate a model:[/dim]  [cyan]devflow generate model <Name> --fields \"field:type\"[/cyan]\n"
        "[dim]Run health check:[/dim]  [cyan]devflow health[/cyan]\n"
        "[dim]View all guides:[/dim]  [cyan]devflow guide[/cyan]\n"
...
def _render_outside_project() -> None:
    """Render the minimal welcome panel when outside a DevFlow project."""
    from devflow import __version__

    content = (
        f"[bold cyan]DevFlow[/bold cyan] v{__version__} — Automated FastAPI Scaffolding CLI\n\n"
        "Start a new project:\n"
        "  [bold cyan]devflow init <project-name>[/bold cyan]\n\n"
        "Browse guides:\n"
        "  [bold cyan]devflow guide[/bold cyan]\n\n"
        "[dim]No .devflow.json found in this directory.[/dim]"
    )
    console.print(
        Panel(
            content,
            title="[bold cyan]⚡ Welcome to DevFlow[/bold cyan]",
            border_style="cyan",
            expand=False,
        )
    )
...
def welcome_dashboard() -> None:
    """Render the DevFlow welcome dashboard.

    Inside a project: full stats, connection status, action-required items.
    Outside a project: minimal welcome pointing to devflow init and devflow guide.
    ...
    try:
        ...
        console.print(
            Panel(
                ...
                "Run [bold]devflow config show[/bold] for more info.[/yellow]",
                border_style="yellow",
            )
        )
