"""DevFlow fallback engine — renders core/fallback.py into the user's project.

This module is part of the DevFlow CLI (not the user's app).  It renders
the ``fallback_core.py.j2`` template into ``<output_dir>/core/fallback.py``
in the user's project when ``devflow cloud fallback enable`` is run.
...
- X-DevFlow-Mode response header injection
"""
